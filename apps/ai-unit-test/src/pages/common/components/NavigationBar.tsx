...
const NavigationBar: React.FC = () => {
  ...
  ) as { isLogin: unknown; setIsLogin: (v: unknown) => void; setUserData: (u: { id: number; name?: string }) => void };
  const { cart, initCart } = useCartStore(state =>
    pick(state, 'cart', 'initCart'),
  ) as { cart: Record<string, unknown>; initCart: (userId: number) => void };
  ...
  const { data, remove } = useProfile({
    config: {
      onSuccess: (profile: { id: number; name?: string }) => {
        setUserData(profile);
        initCart(profile.id);
      },
      enabled: !!isLogin,
